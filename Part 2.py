...
# (1 Mark) High resolution computation of the set by decreasing the mgrid spacing and zooming to another
...
# (1 Mark) Modify the code so to show a Julia set rather than the Mandelbrot set
for i in range(200):
    # Compute the new values of z: z^2 + x
    zs_ = (zs * zs) + (0.285 + 0.01*z)
    # Have we diverged with this new value?
    not_diverged = torch.abs(zs_) < 4.0
    # Update variables to compute
    ns += not_diverged.type(torch.FloatTensor)
